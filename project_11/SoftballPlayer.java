...
public abstract class SoftballPlayer implements Comparable<SoftballPlayer>
{
  ...
  /**
   * Returns a negative if the number of object one is less, zero
   * if the numbers are the same, and positive if the number of the
   * the first is bigger. 
   * 
   * @param other for SoftballPlayer object. 
   * @return comparison.  
   */
   public int compareTo(SoftballPlayer other)
   {
      if (Double.parseDouble(this.getNumber()) 
            < Double.parseDouble(other.getNumber()))
      {
         return -1;
      }
